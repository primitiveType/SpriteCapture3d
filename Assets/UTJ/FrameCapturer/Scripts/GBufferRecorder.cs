...
namespace UTJ.FrameCapturer
{
    ...
    public class GBufferRecorder : RecorderBase
    {
        #region fields
        [SerializeField]
        MovieEncoderConfigs m_encoderConfigs = new MovieEncoderConfigs(MovieEncoder.Type.Exr);
        [SerializeField] 
        FrameBufferConponents m_fbComponents = FrameBufferConponents.defaultValue;
    ...
    class BufferArrayRecorder
    {
        ...
        public BufferArrayRecorder(RenderTexture[] rt, int ch, string name, int tf)
        {
            m_rt = rt;
            m_channels = ch;
            m_name = name;
        ...
        {
            string path = p.GetFullPath() + "/" + m_name;
            c.Setup(m_rt[0].width, m_rt[0].height, m_channels, m_targetFramerate);
            m_encoder = MovieEncoder.Create(c, path);
            return m_encoder != null && m_encoder.IsValid();
        }
        ...
        public void Update(double time)
        {
            if (m_encoder != null)
            {
                fcAPI.fcLock(m_rt[currentFrame], (data, fmt) =>
                {
                    m_encoder.AddVideoFrame(data, fmt, time);
                });
                currentFrame++;
